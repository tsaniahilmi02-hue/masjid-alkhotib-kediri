...
const firebaseConfig = {
    apiKey: "AIzaSyCwmpidD_y2rfxNTlHMjHSgrOXHpgQoWQs",
    authDomain: "masjid-alkhotib2.firebaseapp.com",
    databaseURL: "https://masjid-alkhotib2-default-rtdb.asia-southeast1.firebasedatabase.app",
    projectId: "masjid-alkhotib2",
    storageBucket: "masjid-alkhotib2.firebasestorage.app",
    messagingSenderId: "321584877752",
    appId: "1:321584877752:web:ba8b12166b0efb0322644d"
};
...
async function saveKegiatan() {
    ...
    let data = await loadData('kegiatan');
    ...
    await saveData('kegiatan', data);
    closeModal('kegiatanModal');
    await renderTable('kegiatan');
    await updateDashboard();
}
...
async function deleteKegiatan(id) {
    ...
    const data = (await loadData('kegiatan')).filter(d => d.id !== id);
    await saveData('kegiatan', data);
...
async function saveProgram() {
    ...
    let data = await loadData('program');
    ...
    await saveData('program', data);
    closeModal('programModal');
    await renderTable('program');
    await updateDashboard();
}
...
async function saveBerita() {
    ...
    const saveItem = async (fotoUrl) => {
        let data = await loadData('berita');
        ...
        await saveData('berita', data);
        closeModal('beritaModal');
        await renderTable('berita');
        await updateDashboard();
    };
    ...
    if (fotoFile) {
        ...
        reader.onload = async function (e) {
            await saveItem(e.target.result);
        };
...
async function editBerita(id) {
    const data = await loadData('berita');
    const item = data.find(d => d.id === id);
    if (!item) return;
    editIds.berita = id;
...
async function saveGaleri() {
    ...
    reader.onload = async function (e) {
        ...
        let data = await loadData('galeri');
        ...
        await saveData('galeri', data);
        closeModal('galeriModal');
        await renderTable('galeri');
        await updateDashboard();
    };
...
async function savePengurus() {
    ...
    let data = await loadData('pengurus');
    ...
    await saveData('pengurus', data);
    closeModal('pengurusModal');
    await renderTable('pengurus');
}
...
async function loadAllTables() {
    await Promise.all(Object.keys(DATA_KEYS).map(type => renderTable(type)));
}

async function updateDashboard() {
    const berita = await loadData('berita');
    const kegiatan = await loadData('kegiatan');
    const program = await loadData('program');
    const donasi = await loadData('donasi');
    const pemasukan = await loadData('pemasukan');
    const pengeluaran = await loadData('pengeluaran');
...
async function loadData(type) {
    const ref = database.ref(DATA_KEYS[type]);
